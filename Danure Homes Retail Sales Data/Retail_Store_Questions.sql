...
SELECT Customer_Name, ROUND(SUM(sales), 3) AS TotalOrderValue
FROM superstore
...
SELECT Product_ID, ROUND(AVG(sales), 3) AS Average_Sales
FROM superstore
GROUP BY Product_ID
...
WITH cte AS (
    ...
    FROM superstore s
    INNER JOIN cte ON s.City = cte.City
...
) SELECT h.City, h.highest_order, h.Customer_Name AS highest_order_customer,
    l.lowest_order, l.Customer_Name AS lowest_order_customer
...
SELECT Sub_Category, ROUND(SUM(sales), 3) AS total_quantity
FROM superstore
WHERE Region = 'West'
...
SELECT order_id, COUNT(order_id) AS order_high
FROM superstore
GROUP BY order_id
ORDER BY order_high DESC
LIMIT 1;

-- Q8) Which order has the highest cumulative value?
SELECT order_id, ROUND(SUM(sales), 3) AS order_value
FROM superstore
GROUP BY order_id
...
SELECT segment, COUNT(order_id) AS num_order
FROM superstore
...
SELECT city, ROUND(SUM(sales), 3) AS TotalSales
FROM superstore
GROUP BY city
...
WITH cte AS (
    SELECT Customer_ID, Customer_Name, Order_ID, Order_Date, ROUND(SUM(sales), 3) AS order_value,
        DATEDIFF(Order_Date, LAG(Order_Date) OVER (PARTITION BY Customer_ID ORDER BY Order_Date ASC)) AS date_diff
    FROM superstore
    GROUP BY Customer_ID, Customer_Name, Order_ID, Order_Date
    HAVING SUM(sales) > 50
...
-- Q14) Find the maximum number of days for which total sales on each day kept rising?
 WITH sales_sequence AS (
    ...
    SELECT s1.Order_Date, COUNT(*) AS rising_day_count
    FROM sales_sequence s1
    INNER JOIN sales_sequence s2 ON s1.TotalSales < s2.TotalSales AND s1.rn < s2.rn
    GROUP BY s1.Order_Date
)SELECT MAX(rising_day_count) AS max_rising_days
FROM rising_days;
